<template>
 <div class="done-container">
   <el-card class="box-card">
     ...
     <div class="text item">
       <queryForm @queryClick="queryClick" :done="true" />
       <el-table v-loading="tabloading" :data="tableData" border tooltip-effect="dark" :element-loading-text="loadingText">
         ...
         <el-table-column label="状态" align="center">
           <template v-slot="scope">
            <el-tag :type="scope.row.type === 3 ? 'warning':'—'">{{scope.row.type === 3 ? '已办理' :'办理中'}}</el-tag>
           </template>
         </el-table-column>
     ...
     <ConDialog ref="cdialog" @getData="getData"/>
   </el-card>
 </div>
</template>
...
import ConDialog from "./components/condialog";
import {getDoneList} from './api/conven'
...
export default {
  ...
  methods:{
    ...
    look(id){
      this.$refs.cdialog.showEdit(id,3)
    },
    SizeChange(val){
      this.pageSize = val;
