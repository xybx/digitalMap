<template>
  <div class="deview-container">
    <el-card class="box-card">
      ...
      <ConDialog ref="cdialog" :deview="deview" @getData="getData"/>
    </el-card>
  </div>
</template>
...
import ConDialog from "@/views/conven/components/condialog";
import {mapGetters} from "vuex";
import {getDeviewList} from "@/views/conven/api/conven";
export default {
  name: "deview",
  data() {
    return {
  ...
  methods: {
    async getData(form){
      ...
      if(form?.repconten || form?.startdate){
        Object.assign(data,{
    ...
    look(id){
      this.$refs.cdialog.showEdit(id,1)
    },
    SizeChange(val) {
      this.pageSize = val;
