  ...
  methods: {
    ...
    async showData(){
      ...
      if(this.partype == 1 || this.partype == 3){
        Object.assign(upform,{membername:res.data.membername,phonenumber:res.data.memberphone,memberid:res.data.masterid,familyid:res.data.familyid})
        sessionStorage.setItem('upform',JSON.stringify(upform))
      }
      if(res.data.housepropertycard && res.data.housepropertycard !== ''){
    ...
    saveData(){
      this.$refs.ownerData.validate(async (valid)=>{
        if(valid){
          ...
          let data = Object.assign({},this.ownerData,{houselongitude:arr[0],housedimensionality:arr[1]})
