  ...
  <div class="letterinfo">
    ...
    <el-form ref="letterData" :model="letterData" :rules="rules" status-icon>
        <el-form-item label="是否信访重点关注人员" prop="attention">
          <el-select v-model="letterData.attention" placeholder="请选择是否信访重点关注人员" :disabled="disabled" @change="seltChange">
            <el-option :value="0" label="否"></el-option>
            <el-option :value="1" label="是"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-button v-if="partype != 2" type="primary" size="medium" :disabled="btndisabled" @click="saveLetter">保 存</el-button>
          <el-button v-if="partype != 2" plain size="medium" :disabled="btndisabled" @click="resetLetter">重 置</el-button>
    ...
    <div class="listbox" v-if="partype === 2">
      <ul class="letterlist">
          <li v-for="item in listData" :key="item.id">
            <span><label>信访诉求：</label>{{item}}</span>
...
<script>
import {getLetterDetail,putLetterData} from '../api/levrest'
...
export default {
  ...
  data() {
    return {
      ...
      rules:{
        attention:[{required:true,trigger:'change',message:'请选择是否信访重点关注人员'}]
      },
      listData:[],
      disabled:false,
      familydata:{}
    ...
    saveLetter(){
      ...
      this.$refs.letterData.validate(async (valid)=>{
        if(valid){
          ...
          Object.assign(params,{memberid:this.cardid ? this.cardid : masterid})
