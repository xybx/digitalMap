...
import {getServerurl} from '../api/levrest'
...
export default {
  ...
  methods: {
   ...
   async saveToGisFamilyAddress(pt) {
          ...
          //查询该户是否已存在农田位置数据，存在先删除
          var querytask = new QueryTask(app.jtwzmserverurl);
          var queryObject = new Query();
     queryObject.where = "1=1  and hbh='" + app.baseinfo.familyid+"'";
     queryObject.outFields = ["*"];
     queryObject.returnGeometry = true;
     queryObject.outSpatialReference = new SpatialReference({ wkid: 4326 });
          query.executeQueryJSON(app.jtwzmserverurl,queryObject).then(function (result) {
            if (result != null) {
              if (result.features.length > 0) {
                var feature = result.features[0];
                featurelayer.applyEdits({
