<template>
  <div class="cominfo">
    <el-card class="box-card">
      ...
      <div class="text item">
        ...
        <el-table v-loading="tabloading" :data="tableData" border tooltip-effect="dark" :element-loading-text="loadingText">
          ...
          <el-table-column label="类型" align="center" show-overflow-tooltip>
            <template v-slot="scope">
              {{ scope.row.type == 1 ? '企业安全生产' : '企业环保资料' }}
            </template>
          </el-table-column>
      ...
      <comdialog ref="cominfo" :info="true" @getData="getData"/>
    </el-card>
  </div>
</template>
<script>
import {delcominfo, getcominfolist} from "@/views/safeProduct/manage/api/comage";
...
export default {
  ...
  methods: {
    async getData(){
      ...
      let res = await getcominfolist(data)
      if(res.code === 200){
