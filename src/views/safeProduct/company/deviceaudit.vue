	...
	<div class="deviceaudit">
        ...
        <queryForm :audit="true" @queryClick="queryClick"/>
            ...
            label="序号"
					...
					<el-table-column label="企业设备名称" align="center" show-overflow-tooltip>
						<template v-slot="scope">
							{{ scope.row.name ? scope.row.name : '暂无企业设备名称' }}
						</template>
					</el-table-column>
...
<script>
import { getUserList, passDevice } from './api/device'
    ...
    queryClick(form){
      ...
      this.getData(form)
		...
		async pass(pid) {
			...
			let res = await passDevice(qs.stringify(form))
			console.log(res)
			if (res.code === 200) {
