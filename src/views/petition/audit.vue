<template>
  <div class="audit">
    <el-card class="box-card">
      ...
      <div class="text item">
        ...
        <el-table v-loading="tabloading" :data="tableData" border tooltip-effect="dark" :element-loading-text="loadingText">
          ...
          <el-table-column label="操作" align="center">
            <template v-slot="scope">
              <el-button type="danger" @click="acceptClick(scope.row.pid)">受理</el-button>
              <el-button type="danger" plain @click="rejectClick(scope.row.pid)">驳回</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
      ...
      <petdialog :audit="true" ref="pdialog" @getData="getData"/>
      <rejectdialog ref="rdialog" @getData="getData"/>
    </el-card>
  </div>
</template>
<script>
...
import {getpendList} from './api/petion'
export default {
  name:'audit',
  data(){
    return {
  ...
  methods:{
    async getData(){
      ...
      let res = await getpendList(data)
      if(res.code ===200){
    ...
    acceptClick(pid){
      this.$refs.pdialog.showEdit(pid,1)
    },
    rejectClick(pid){
      this.$refs.rdialog.showreject(pid)
    },
    queryClick(form){
      this.formData = form
      this.pageNo = 1
