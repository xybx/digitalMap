...
<div class="remian">
  <el-card class="box-card">
      ...
      <el-table v-loading="tabloading" :data="tableData" border tooltip-effect="dark" :element-loading-text="loadingText">
        ...
        <el-table-column prop="appeal" label="主要诉求" align="center">
          <template v-slot="scope">
            <i v-if="scope.row.supervise === 1" class="iconfont icon-jiarehuoyan"><u>督</u></i>
        ...
        <el-table-column label="操作" align="center">
          <template v-slot="scope">
            <el-button type="danger" @click="handleClick(scope.row.pid,scope.row.supervise)">办理</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <petdialog :remain="true" ref="pdialog" @getData="getData"/>
    <sreplydialog ref="sreply" @getData="getData" />
  </el-card>
</div>
</template>
<script>
...
import sreplydialog from "./components/sreplydialog";
import {getneedtoList} from './api/petion'
export default {
  name:'remain',
  data(){
    return {
  ...
  methods:{
    async getData(){
      ...
      let res = await getneedtoList(data)
      if(res.code ===200){
    ...
    handleClick(pid,sid){
      if(sid == 1){
        this.$refs.sreply.showreply(pid)
      }else {
        this.$refs.pdialog.showEdit(pid,2)
      }
    },
    queryClick(form){
      this.formData = form
