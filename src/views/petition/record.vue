<template>
  <div class="record">
    <el-card class="box-card">
      ...
      <div class="text item">
        ...
        <el-table v-loading="tabloading" :data="tableData" border tooltip-effect="dark" :element-loading-text="loadingText">
          ...
          <el-table-column label="操作" align="center">
            <template v-slot="scope">
              <el-button type="danger" @click="lookClick(scope.row.pid)">查看</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
      ...
      <petdialog :record="true" ref="pdialog" @getData="getData"/>
    </el-card>
  </div>
</template>
<script>
...
import {getareaList} from "@/views/petition/api/petion";
export default {
  name:'',
  data(){
    return {
  ...
  methods:{
    async getData(){
      ...
      let res = await getareaList(data)
      if(res.code ===200){
    ...
    lookClick(pid){
      this.$refs.pdialog.showEdit(pid,5)
    },
    queryClick(form){
      this.formData = form
      this.pageNo = 1
