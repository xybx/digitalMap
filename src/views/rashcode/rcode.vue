    ...
    <el-row :gutter="5">
      ...
      <el-col :lg="16" :md="16" :sm="24" :xl="16" :xs="24">
        <el-card class="box-card animate__animated animate__fadeInRight">
          ...
          <div class="text item">
            <queryForm ref="queryd" @queryClick="queryClick"/>
              ...
              <el-table-column v-for="item in tableColumns" :key="item.prop" align="center" :width="item.prop== 'checkresult' && item.prop == 'pannierphone' ? '150px' : ''" show-overflow-tooltip :label="item.label">
                <template #default="{ row, $index }">
                  <template v-if="item.prop == 'result'">
                    <el-tag :type="row[item.prop] == 2 ? 'danger' : 'success'">{{row[item.prop] == 2 ? '未通过' :'通过'}}</el-tag>
                  </template>
                  <template v-else>
...
import {getListData, getTotalData} from "@/views/rashcode/api/rcode";
export default {
  name: "rcode",
  data(){
    return {
  ...
  methods:{
    ...
    async getData(){
      ...
      let res = await getListData(data)
    ...
    async getTotal(){
      ...
      this.listData = [
        {value:data.all, name:'总人数'},
        {value:data.bendi, name:'博山本地人数'},
        {value:data.waidi, name:'非博山本地人数'},
      ]
