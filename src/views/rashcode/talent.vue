...
import {getTalentList, getTalentTotal} from "./api/rcode";
export default {
  name: "talent",
  data(){
    return {
  ...
  methods:{
    ...
    async getData(){
      ...
      let res = await getTalentList(data)
    ...
    async getTotal(){
      let {data} = await getTalentTotal()
      this.listData = [
        {value:data.all, name:'人才总数'},
        {value:data.shi, name:'属于人才数'},
        {value:data.bushi, name:'不属于人才数'},
      ]
