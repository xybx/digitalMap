...
<template>
  <el-dialog :visible.sync="dialogVisible" :title="title" :width="tcard == 'codefth' || tcard == 'codevth' ? '32%' : '40%'" :top="tcard == 'msgrule' ? '10%' : '3%'" :before-close="handleClose" :append-to-body="true" :close-on-click-modal="false" center>
    <el-form ref="frestData" :model="frestData" :rules="rules" status-icon label-width="120px">
        ...
        <el-form-item label="所属镇街" prop="areaid" v-if="tcard == 'codeth'">
        ...
        <el-form-item label="所属镇街" prop="villageId" v-if="(tcard == 'codest'&&keys!== 2)">
          <el-select v-model="frestData.villageId" placeholder="请选择所属镇街" clearable :disabled="disabled">
            <el-option v-for="item in areadata" :key="item.pid" :value="item.pid" :label="item.areaname"></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="街镇编号" prop="villageNumber" v-if="tcard == 'codest'&&keys!== 2">
          <el-input v-model="frestData.villageNumber" placeholder="请输入街镇编号" :disabled="disabled"/>
        </el-form-item>
        <el-form-item label="所属村居" prop="villageid" v-if="tcard == 'codeth'">
