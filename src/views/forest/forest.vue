        ...
        <el-aside>
          <div class="vflex">
            <el-radio-group v-model="tabCard" class="radiogroup" @change="tabChange" size="—">
              <el-radio-button label="codest"><i class="iconfont icon-changsuoma"></i>场所码管理</el-radio-button>
              <el-radio-button label="codend"><i class="iconfont icon-shenfenma"></i>人员通行证管理</el-radio-button>
              ...
              <template v-if="level == 2">
                <el-radio-button label="codevth"><i class="iconfont icon-pashan"></i>登记事项</el-radio-button>
              </template>
            </el-radio-group>
          </div>
        </el-aside>
        <el-main>
          <div class="text item">
            ...
            <el-table v-loading="tabloading" :data="tableData" border tooltip-effect="dark" element-loading-background="rgba(255, 255, 255, 1)" :element-loading-text="loadingText">
              <el-table-column type="index" prop="pid" label="序号" align="center" width="60px"></el-table-column>
              ...
              <el-table-column label="操作" :width="tabCard== 'codend' ? '420px' : '260px'" align="center">
                <template #default="{row,$index}">
                  <el-button v-if="tabCard== 'codest'" type="warning" plain @click.stop="codeClick(row.pid)">生成场所码</el-button>
                  <el-button v-if="tabCard== 'codend'" type="success" plain @click.stop="createGatePass(row.pid)">生成人员通行证</el-button>
                  <el-button v-if="tabCard== 'codend'" type="warning" plain @click.stop="lookcode(row.pid)">查看人员通行证</el-button>
            ...
            <el-pagination background :current-page="page" :page-size="pageSize" :page-sizes="pageSizes" :layout="layout" :total="total" @size-change="SizeChange" @current-change="CurrentChange"></el-pagination>
          </div>
        </el-main>
      </el-container>
    </el-card>
<!--    <frestdialog ref="frestlog" @getData="getData" :tcard="tabCard" />-->
    ...
    <placeCodeTypeDialog ref="placeCodeTypeRef" @getData="getData" :tcard="tabCard" />
    <matterDialog ref="matterRef" @getData="getData" :tcard="tabCard" />
    <gatePassDialog ref="gatePassRef" :tcard="tabCard" />
  </div>
</template>

<script>
export { default } from './js/forest'
</script>
