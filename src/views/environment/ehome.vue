...
import {getpies, getTotals} from "@/views/environment/api/eindex";
import {gethasDoneList, getmyselfList, getpendingList, getTodoList} from "@/views/environment/api/environment";
export default {
  name: "ehome",
  data(){
    return {
  ...
  methods:{
    async getData(){
      ...
      let res = this.states == 1 ? await getTodoList(data) : this.states == 2 ? await getpendingList(data) : this.states == 3 ? await getmyselfList(data) : await gethasDoneList(data)
    ...
    cardClick(i){
      this.states = i == 0 ? 1 : i==1 ? 2 : i == 2 ? 3 : 4
      this.getData()
    },
    getPiechart(arr){
    ...
    lookClick(pid){
      this.$refs.envrion.showData(pid,this.states)
    },
    ...
    exportClick(){
      ...
      a.download = '文件下载';
      a.href = `${this.$baseUrl}/livingexcel/livingexcel?areaid=${this.userData.areaid}&uid=${this.userData.pid}&type=${this.states}`;
