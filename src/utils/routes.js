...
export function convertRouter(asyncRoutes) {
  return asyncRoutes.map((route) => {
    if (route.component) {
      ...
      } else if(route.component === 'Szdp'){
        route.component = (resolve) => require(['@/vab/szdp'], resolve)
      }else if(route.component === 'Leader'){
        route.component = (resolve) => require(['@/vab/leader'], resolve)
...
export function toLoginRoute(currentPath) {
  if (recordRoute && currentPath !== 0)
    return {
      path: '/login',
      query: { areaid: currentPath },
      replace: true,
    }
  else return { path: '/login',query:{ areaid: currentPath }, replace: true }
}
